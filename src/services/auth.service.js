...
const sendAuthentication = async (user, oldRefreshToken) => {
...
export const login = async ({ email, password }) => {
  ...
  return sendAuthentication(user);
};
...
export const activate = async ({ activationToken }) => {
  ...
  await userService.setActivationToken(user, null);
};
...
export const refresh = async ({ refreshToken }) => {
  const foundRefreshToken = await tokenService.findTokenByToken(refreshToken);
  ...
  try {
    ...
    return sendAuthentication(user, refreshToken);
  } catch (error) {
